...
//  SpotTheScientist
//
//  Created by Yashraj jadhav on 16/01/23.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        guard let trackingImages = ARReferenceImage.referenceImages(inGroupNamed: "scientists", bundle: nil) else {
            fatalError("Couldn't load the tracking images  ")
        }
...
//        print(scientist.bio)
    ...
    func loadData ()
    {
        ...
        guard let lodadedScientists = try? decoder.decode([String: Scientist].self, from: data) else {
            fatalError("Unable to parse JSON")
        }
        
        scientists = lodadedScientists
    }
